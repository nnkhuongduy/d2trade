...
class VirtualizedTable extends PureComponent {
  ...
  cellRenderer = ({ dataKey, rowData, cellData }) => {
    ...
    return (
      ...
      >
        {/* {dataKey === 'avatar' ?
          <Link to={`users/${rowData.steam_id}`}><Avatar src={cellData} /></Link> :
          dataKey === 'date' ?
            moment(cellData).tz('Asia/Ho_Chi_Minh').format('DD/MM/YYYY') :
            dataKey === 'user_balance' ?
              `- ${cellData.toLocaleString()}` :
              dataKey === 'profit' ?
                <span style={{ color: 'green' }}>+ {cellData.toLocaleString()}</span> :
                cellData
        } */}
        {cellData}
      </TableCell>
